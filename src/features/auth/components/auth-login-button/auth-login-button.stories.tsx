...
const meta = preview.meta({
    component: AuthLoginButton,
    tags: ["autodocs"],
    args: {
        provider: AUTH_PROVIDER.GOOGLE,
        children: "Sign in with Google",
        onClick: fn(),
        onLoggingInChange: fn(),
    },
    parameters: {
        a11y: {
            test: "error",
    ...
    argTypes: {
        ...
        onLoggingInChange: {
            description: "Callback when logging in state changes",
            table: {
                type: {
                    summary: "(isLoggingIn: boolean) => void",
                },
            },
        },
    },
...
Google.test(
    "should call onLoggingInChange when logging in state changes",
    async ({ canvas, userEvent, args }) => {
        const button = canvas.getByRole("button");
        await userEvent.click(button);

        await waitFor(() => {
            expect(args.onLoggingInChange).toHaveBeenCalledWith(true);
        });
    },
);
